...
class FeedbackGate(object):
    ...
    def get_feedback_from(self, source):
        ...
        try:
            fbk = (
                self.db.last_feedback[source]
                if self.last_fbk_entries
                else self.db.feedback_trail[source]
            )
        except KeyError:
            raise
        else:
    ...
    def iter_entries(self, source=None):
        ...
        """
        for item in self.db.iter_feedback_entries(
            last=self.last_fbk_entries, source=source
        ):
            yield item
    ...
    def sources_names(self):
        ...
        """
        fbk_db = (
            self.db.last_feedback if self.last_fbk_entries else self.db.feedback_trail
        )
...
class Database(object):
    ...
    def _sql_handler(self):
        if os.path.isfile(self.fmk_db_path):
            connection = sqlite3.connect(
                self.fmk_db_path, detect_types=sqlite3.PARSE_DECLTYPES
            )
    ...
    def submit_sql_stmt(
        self, stmt, params=None, outcome_type: Optional[int] = None, error_msg=""
    ):
        """
    ...
    def start(self):
        ...
        self.feedback_trail = (
            {}
        )  # store feedback entries for self.feedback_trail_time_window
        self.feedback_trail_init_ts = None
        self.feedback_trail_time_window = self.FEEDBACK_TRAIL_TIME_WINDOW
        ...
        if not sqlite3_module:
            print(
                "/!\\ WARNING /!\\: Fuddly's FmkDB unavailable because python-sqlite3 is not installed!"
            )
            return False

        self._sql_handler_thread = threading.Thread(
            None, self._sql_handler, "db_handler"
        )
    ...
    def execute_sql_statement(self, sql_stmt, params=None):
        return self.submit_sql_stmt(
            sql_stmt, params=params, outcome_type=Database.OUTCOME_DATA
        )
    ...
    def insert_data(
        ...
        if self._data_id is None:
            d_id = self.submit_sql_stmt(
                stmt,
                params=params,
                outcome_type=Database.OUTCOME_ROWID,
                error_msg=err_msg,
            )
            self._data_id = d_id
        else:
    ...
    def insert_async_data(
        ...
        stmt = (
            "INSERT INTO ASYNC_DATA(CURRENT_DATA_ID,TYPE,DM_NAME,CONTENT,SIZE,SENT_DATE,"
            "TARGET,PRJ_NAME)"
            " VALUES(?,?,?,?,?,?,?,?)"
        )
        ...
        if self._current_sent_date is not None:
            ...
            if (
                sent_date - self._current_sent_date
            ).total_seconds() > self.config.async_data.after_data_id:
                data_id = None
            else:
    ...
    def insert_steps(
        self, data_id, step_id, dmaker_type, dmaker_name, data_id_src, user_input, info
    ):
        if not self.enabled:
            return None
    ...
    def insert_feedback(self, data_id, source, timestamp, content, status_code=None):
        ...
        if (
            ...
            and timestamp - self.feedback_trail_init_ts
            > timedelta(seconds=self.feedback_trail_time_window)
        ):
            self.feedback_trail = {}
            self.feedback_trail_init_ts = timestamp
        ...
        fbk_entry = {"timestamp": timestamp, "content": content, "status": status_code}

        self.last_feedback[source].append(fbk_entry)
        self.feedback_trail[source].append(fbk_entry)

        if self.current_project:
            self.current_project.trigger_feedback_handlers(
                source, timestamp, content, status_code
            )
    ...
    def iter_feedback_entries(self, last=True, source=None):
        ...
        if source is None:
            ...
            fbk_from_src = (
                self.last_feedback[source] if last else self.feedback_trail[source]
            )
    ...
    def insert_comment(self, data_id, content, date):
        ...
        stmt = "INSERT INTO COMMENTS(DATA_ID,CONTENT,DATE)" " VALUES(?,?,?)"
    ...
    def insert_fmk_info(self, data_id, content, date, error=False):
        ...
        stmt = "INSERT INTO FMKINFO(DATA_ID,CONTENT,DATE,ERROR)" " VALUES(?,?,?,?)"
    ...
    def insert_analysis(self, data_id, content, date, impact=False):
        ...
        stmt = "INSERT INTO ANALYSIS(DATA_ID,CONTENT,DATE,IMPACT)" " VALUES(?,?,?,?)"
    ...
    def fetch_data(self, start_id=1, end_id=-1):
        ...
        stmt = """
            ...
            WHERE DATA.ID >= {sid:d} {ign_eid:s} AND DATA.ID <= {eid:d}
            """.format(
            sid=start_id, eid=end_id, ign_eid=ign_end_id
        )
    ...
    def check_data_existence(self, data_id, colorized=True):
        ...
        data = self.execute_sql_statement(
            "SELECT * FROM DATA " "WHERE ID == {data_id:d};".format(data_id=data_id)
        )

        if not data:
            print(
                colorize(
                    "*** ERROR: The provided DATA ID does not exist ***",
                    rgb=Color.ERROR,
                )
            )
    ...
    def display_data_info(
        ...
        def search_dm(data_model_name, load_arg):
            for dm in dm_list:
                ...
                print(
                    colorize(
                        "*** ERROR: No available data model matching this database entry "
        ...
        msg += colorize(
            "]".ljust(page_width - (page_width - 20) - len(data_id_pattern), "="),
        ...
        def handle_dmaker(
            dmk_pattern, info, dmk_type, dmk_name, name_sep_sz, ui, id_src=None
        ):
            msg = ""
            ...
            for _, step_id, dmk_type, dmk_name, id_src, ui, info in steps:
                if first_pass:
                    ...
                    if dmk_type != data_type:
                        ...
                        msg += handle_dmaker(
                            "Disruptor", info, dmk_type, dmk_name, len(data_type), ui
                        )
                    else:
                        msg += handle_dmaker(
                            "Generator",
                    ...
                    msg += handle_dmaker(
                        "Disruptor", info, dmk_type, dmk_name, name_sep_sz, ui
                    )
                sid += 1
            msg += colorize("\n" + line_pattern, rgb=Color.NEWLOGENTRY)
            prt(msg)
        ...
        for idx, info in enumerate(analysis_records, start=1):
            ...
            if impact:
                msg += colorize(
                    "Data triggered an unexpected behavior", rgb=Color.ANALYSIS_IMPACT
                )
            else:
                msg += colorize(
                    "Data did not trigger an unexpected behavior",
                    rgb=Color.ANALYSIS_NO_IMPACT,
                )
            if content:
        ...
        if with_data:
            ...
            if decode_data:
                atom_name = (
                    data_type.lower() if user_atom_name is None else user_atom_name
                )
                msg += decoder_func(
                    data_content, atom_name=atom_name, colorized=colorized
                )
            else:
                data_content = gr.unconvert_from_internal_repr(data_content)
                data_content = self._handle_binary_content(
                    data_content, sz_limit=limit_data_sz, raw=raw, colorized=colorized
                )
                msg += data_content
            msg += colorize("\n" + line_pattern, rgb=Color.NEWLOGENTRY)
        ...
        if with_async_data and async_data:
            for (
                ...
                add_info = (
                    f" --> sent before Data ID #{data_id}"
                    if curr_data_id is None
                    else ""
                )
                date_str = (
                    async_date.strftime("%d/%m/%Y - %H:%M:%S.%f")
                    if async_date
                    else "None"
                )
                msg += colorize(
                ...
                content = self._handle_binary_content(
                    content, sz_limit=limit_data_sz, raw=raw, colorized=colorized
                )
                msg += content
        ...
        if with_fbk:
            for src, tstamp, status, content in feedback:
                formatted_ts = (
                    None
                    if tstamp is None
                    else tstamp.strftime("%d/%m/%Y - %H:%M:%S.%f")
                )
                msg += colorize("\n Status(", rgb=Color.FMKINFOGROUP)
                msg += colorize("{!s}".format(src), rgb=Color.FMKSUBINFO)
                msg += colorize(" | ", rgb=Color.FMKINFOGROUP)
                msg += colorize("{!s}".format(formatted_ts), rgb=Color.FMKSUBINFO)
                msg += colorize(")", rgb=Color.FMKINFOGROUP)
                msg += colorize(" = {!s}".format(status), rgb=Color.FMKSUBINFO)
                if content:
                    if decode_fbk:
                        msg += fbk_decoder_func(
                            content, atom_name=user_fbk_atom_name, colorized=colorized
                        )
                    else:
                        content = gr.unconvert_from_internal_repr(content)
                        content = self._handle_binary_content(
    ...
    def display_data_info_by_date(
        ...
        if prj_name:
            ...
            records = self.execute_sql_statement(
                "SELECT ID FROM DATA " "WHERE ? <= SENT_DATE and SENT_DATE <= ?;",
                params=(start, end),
            )
        ...
        if records:
            for rec in records:
                ...
                self.display_data_info(
                    ...
                    colorized=colorized,
                    decoding_hints=decoding_hints,
                    dm_list=dm_list,
                )
        else:
            print(
                colorize(
                    "*** ERROR: No data found between {!s} and {!s} ***".format(
                        start, end
                    ),
                    rgb=Color.ERROR,
                )
    ...
    def display_data_info_by_range(
        self,
        first_id,
        last_id,
        with_data=False,
        ...
        if prj_name:
            records = self.execute_sql_statement(
                "SELECT ID FROM DATA " "WHERE ? <= ID and ID <= ? and PRJ_NAME == ?;",
                params=(first_id, last_id, prj_name),
            )
        else:
            records = self.execute_sql_statement(
                "SELECT ID FROM DATA " "WHERE ? <= ID and ID <= ?;",
    ...
    def display_stats(self, colorized=True):
        ...
        if records:
            ...
            for rec in records:
                ...
                print(
                    colorize(
                        format_string.format(data_type, total), rgb=Color.FMKSUBINFO
                    )
                )

        else:
            print(
                colorize("*** ERROR: Statistics are unavailable ***", rgb=Color.ERROR)
            )

        data_records = self.execute_sql_statement("SELECT ID FROM DATA;")
    ...
    def export_data(self, first, last=None, colorized=True):
        ...
        if last is not None:
            records = self.execute_sql_statement(
                ...
                "WHERE {start:d} <= ID and ID <= {end:d};".format(start=first, end=last)
            )
        else:
            records = self.execute_sql_statement(
                "SELECT ID, TYPE, DM_NAME, SENT_DATE, CONTENT FROM DATA "
        ...
        if records:
            ...
            for rec in records:
                ...
                print(
                    colorize(
                        "Data ID #{:d} --> {:s}".format(data_id, export_full_fn),
                        rgb=Color.FMKINFO,
                    )
                )
            ...
            print(
                colorize(
                    "*** ERROR: The provided DATA IDs do not exist ***", rgb=Color.ERROR
                )
            )
    ...
    def remove_data(self, data_id, colorized=True):
        ...
        steps = self.execute_sql_statement(
            "DELETE FROM STEPS " "WHERE DATA_ID == {data_id:d};".format(data_id=data_id)
        )

        data = self.execute_sql_statement(
            "DELETE FROM DATA " "WHERE ID == {data_id:d};".format(data_id=data_id)
        )

        print(
            colorize(
                "*** Data {:d} and all related records have been removed ***".format(
                    data_id
                ),
                rgb=Color.FMKINFO,
            )
    ...
    def shrink_db(self):
        self.execute_sql_statement("VACUUM;")
    ...
    def get_data_with_impact(
        ...
        analysis_records = self.execute_sql_statement(
            "SELECT DATA_ID, CONTENT, DATE, IMPACT FROM ANALYSIS " "ORDER BY DATE DESC;"
        )
        if analysis_records:
        ...
        if fbk_records and prj_records:
                ...
                if data_id in id2fbk:
                    ...
                    if display:
                        if prj != current_prj:
                            ...
                            print(
                                colorize(
                                    "*** Project '{:s}' ***".format(prj),
                                    rgb=Color.FMKINFOGROUP,
                                )
                            )
                        print(
                            colorize(
                                format_string.format("#" + str(data_id), target),
                                rgb=Color.DATAINFO,
                            )
                        )
                        if verbose:
                            for src, status in id2fbk[data_id].items():
                                if src == user_src:
                                    if status[0] == 0:
                                        status_str = (
                                            "User analysis carried out: False Positive"
                                        )
                                    else:
                                        status_str = "User analysis carried out: Impact Confirmed"
                                    color = (
                                        Color.ANALYSIS_FALSEPOSITIVE
                                        if status[0] == 0
                                        else Color.ANALYSIS_CONFIRM
                                    )
                                    print(
                                        colorize(
                                            "       |_ {:s}".format(status_str),
                                            rgb=color,
                                        )
                                    )
                                else:
            ...
            print(
                colorize(
                    "*** No data has negatively impacted a target ***",
                    rgb=Color.FMKINFO,
                )
            )
    ...
    def get_data_without_fbk(
        self, prj_name=None, fbk_src=None, display=True, colorized=True
    ):
        colorize = self._get_color_function(colorized)
        ...
        if fbk_records and prj_records:
            ...
            print(
                colorize(
                    "*** No data has been found for analysis ***", rgb=Color.FMKINFO
                )
            )

        return data_list

    def get_data_with_specific_fbk(
        self, fbk, prj_name=None, fbk_src=None, display=True, colorized=True
    ):
        colorize = self._get_color_function(colorized)
