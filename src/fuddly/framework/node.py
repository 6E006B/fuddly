...
class NodeInternals_TypedValue(NodeInternals_Term):
    ...
    def _update_value_specific(self, value):
        if isinstance(value, int):
            assert isinstance(
                self.value_type, (fvt.INT, fvt.BitField)
            ), f"{self.value_type}"
...
class NodeInternals_NonTerm(NodeInternals):
    ...
    def _get_value(
        ...
        for idx, n in enumerate(node_list):
            ...
            if (
                self.separator is not None
                and isinstance(val, NodeInternals)
                and val.is_attr_set(NodeInternals.AutoSeparator)
                and disabled_node
                and not self.separator.suffix
                and not self.separator.always
            ):
        ...
        if (
            self.separator is not None
            and l
            and isinstance(l[-1], NodeInternals)
    ...
    def absorb(self, blob, constraints, conf, pending_postpone_desc=None):
        ...
        def _try_absorption_with(
            ...
            if (
                reject_with_min_null
                and self.separator is not None
                and self.separator.always
            ):
                # if DEBUG:
...
class Node(object):
    ...
    def absorb(
        self, blob, constraints=AbsCsts(), conf=None, pending_postpone_desc=None
    ):
    ...
    def freeze(
        ...
        if self.env is None:
            print(
                "Warning: freeze() is called on a node that does not have an Env()\n"
        ...
        if (
            ...
            try:
                ...
                if not self.env.csp.var_domain_updated:
                    ...
                    print(
                        f"Warning: no solution found for the current CSP, the generated data will be invalid!\n"
            ...
            for n, i in zip(l, range(nodes_nb)):
                ...
                if node.is_term(conf_tmp):
                    ...
                    if raw_limit is not None and raw_len > raw_limit:
                        print_raw_func(
                            "\_raw: {:s}".format(repr(raw[:raw_limit])),
                            nl=False,
                            log_func=log_func,
                            pretty_print=pretty_print,
                        )
