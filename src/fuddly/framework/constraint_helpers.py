...
class CSP(object):
    ...
    def __init__(
        self,
        constraints: Constraint or List[Constraint] = None,
        highlight_variables=False,
    ):
        assert (
            csp_module
        ), "the CSP backend is disabled because python-constraint module is not installed!"
        ...
        if self._solutions is None or self._exhausted_solutions:
            ...
            try:
                ...
                raise ConstraintError(
                    f"No solution found for this CSP\n --> variables: {self._vars}"
                )
            else:
                self._model = mdl
        else:
