...
class Logger(object):
    ...
    def __init__(
        ...
        term_display_limit=800,
        enable_term_display=True,
        enable_file_logging=False,
        highlight_marked_nodes=False,
    ):
        """
        Args:
        ...
        def init_logfn(
            ...
            if issubclass(x.__class__, Data):
                data = self._handle_binary_content(
                    x.to_bytes(), raw=self.export_raw_data
                )
                ...
                colored_data = data = self._handle_binary_content(
                    x, raw=self.export_raw_data
                )
            self.print_console(
                colored_data,
                nl_before=nl_before,
    ...
    def start(self):
        ...
        if self.name is None:
            raise ValueError(
                "Logger's name shall be provided (either by the framework or at init)"
            )
            ...
            def intern_func(
                ...
                if issubclass(x.__class__, Data):
                    ...
                    colored_data = (
                        x.to_formatted_str() if self._hl_marked_nodes else data
                    )
                    rgb = None
                    style = None
                    no_format_mode = self._hl_marked_nodes
        ...
        self._log_handler_thread = threading.Thread(
            None, self._log_handler, "log_handler"
        )
    ...
    def commit_data_table_entry(self, group_id, prj_name):
        if self._current_data is not None:  # that means data will be recorded
            ...
            init_dmaker = (
                Database.DEFAULT_GTYPE_NAME if init_dmaker is None else init_dmaker[0]
            )
            ...
            for tg_ref, ack_date in self._current_ack_dates.items():
                ...
                for step_id, dmaker in enumerate(
                    self._current_dmaker_list, start=step_id_start
                ):
                    dmaker_type, dmaker_name, user_input = dmaker
                    info = self._current_dmaker_info.get(
                        (dmaker_type, dmaker_name), None
                    )
                    if info is not None:
                        info = "\n".join(info)
                        info = convert_to_internal_repr(info)
                    ui = str(user_input) if bool(user_input) else None
                    self.fmkDB.insert_steps(
                        last_data_id,
                        step_id,
                        dmaker_type,
                        dmaker_name,
    ...
    def log_async_data(
        self,
        data_list: Data | List[Data] | Tuple[Data],
        sent_date,
        target_ref,
        prj_name,
        current_data_id,
    ...
    def collect_feedback(self, content, status_code=None, subref=None, fbk_src=None):
        ...
        with self._tg_fbk_lck:
            self._tg_fbk.append(
                (now, FeedbackSource(fbk_src, subref=subref), content, status_code)
            )
    ...
    def _export_data_func(self, data, suffix=""):
        ...
        export_fname = "{date:s}_{cpt:0>2d}{suff:s}.{ext:s}".format(
