...
class String(VT_Alt):
    ...
    def _enable_fuzz_mode(self, fuzz_magnitude=1.0):
        ...
        ctrl_chars_tc = String.fuzz_cases_ctrl_chars(
            self.knowledge_source, orig_val, sz, self.max_sz, self.codec
        )
        if ctrl_chars_tc:
            add_to_fuzz_list(ctrl_chars_tc)

        c_strings_tc = String.fuzz_cases_c_strings(
            self.knowledge_source, orig_val, sz, fuzz_magnitude
        )
        if c_strings_tc:
            add_to_fuzz_list(c_strings_tc)

        if self.case_sensitive:
            fc_letter_cases = String.fuzz_cases_letter_case(
                self.knowledge_source, orig_val
            )
            if fc_letter_cases:
        ...
        enc_cases = self.encoding_test_cases(
            orig_val, self.max_sz, self.min_sz, self.min_encoded_sz, self.max_encoded_sz
        )
        if enc_cases:
            self.values_fuzzy += enc_cases

        ### Specific test cases added by class that inherits from String()
        extended_fuzz_cases = self.subclass_specific_test_cases(
            self.knowledge_source, orig_val, fuzz_magnitude
        )
        ...
        if val is not None:
            ...
            if self.mini is not None:
                ...
                if self.value_space_size != -1:  # meaning not an INT_str
                    cond1 = (
                        self.mini != 0 or self.maxi != ((1 << self.size) - 1)
                    ) and (
                        self.mini != -(1 << (self.size - 1))
                        or self.maxi != ((1 << (self.size - 1)) - 1)
                    )
                else:
                    cond1 = True
